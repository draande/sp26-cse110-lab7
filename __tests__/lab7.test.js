describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
    allArePopulated = prodItemsData.every(item => 
      item && item.title?.length > 0 && item.price?.toString().length > 0 && item.image?.length > 0
    );
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allButtonsMatch = await page.$$eval('product-item', (items) => {
      return items.every(item => item.shadowRoot.querySelector('button').innerText === 'Remove from Cart');
    });
    expect(allButtonsMatch).toBe(true);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allButtonsReverted = await page.$$eval('product-item', (items) => {
      return items.every(item => item.shadowRoot.querySelector('button').innerText === 'Add to Cart');
    });
    expect(allButtonsReverted).toBe(true);
